...
# Prune the block log to save disk (keep only recent blocks)
block-log-retain-blocks = 0          # 0 = do not maintain a persistent block log
# Or keep a rolling window, e.g. ~2 days of blocks (2 blocks/sec):
# block-log-retain-blocks = 345600
...
block-log-retain-blocks = 0          # keep the full persistent block log (do not prune)
```
