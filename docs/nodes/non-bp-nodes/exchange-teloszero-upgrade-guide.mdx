...
- <https://snapshots.eosusa.io/snapshots/telos/>
- <https://snapshots.telosunlimited.com/>
...
```ini
# Do NOT load the state_history_plugin
# Save disk by not keeping a full block log:
block-log-retain-blocks = 0          # 0 = disable the block log (blocks.log removed after startup)
# Or keep a rolling window instead, e.g. ~2 days of blocks (2 blocks/sec):
# block-log-retain-blocks = 345600   # prune to the N most recent blocks

# Keep transaction traces available briefly for confirmations
# (chain_api_plugin exposes get_transaction_status without full history)
```
...
Start from a snapshot (Step 7). Sync from the [current peers](#appendix-current-mainnet-p2p-peers). A pruned API node reaches head quickly because it does not replay or retain full history.
...
# --- Full block log from genesis ---
# Do NOT set block-log-retain-blocks on a full-history node.
# Omit it entirely to keep the complete block log from genesis (the default).
#   block-log-retain-blocks = 0    -> DISABLES the block log (blocks.log removed after startup)
#   block-log-retain-blocks = N>0  -> prunes the block log to the N most recent blocks
```
...
- **TelosZero Core:** <https://github.com/telosnetwork/teloszero-core>
- **Telos validator / endpoints dashboard:** <https://validators.telos.net/>
- **Node template:** <https://github.com/telosnetwork/node-template>
- **Telos docs:** <https://docs.telos.net/>
- **Telos GitHub:** <https://github.com/telosnetwork>
