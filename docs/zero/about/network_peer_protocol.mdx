...
<img
    src={require('/static/img/graphviz (5).png').default}
    alt="graph 1"
    width="70%"
    height="automatic"
/>
...
1. Node A sends a message to Node B through the Net Plugin (refer to the diagram above).
...
<img
    src={require('/static/img/graphviz (7).png').default}
    alt="graph 3"
    width="70%"
    height="automatic"
/>

In the diagram above, the branch starting at block 52b gets pruned (blocks 52b, 53a, and 53b are invalid) after the LIB advances from node 51 to block 52c then 53c. As the LIB moves through the reversible blocks, they are moved from the Fork DB to the local chain as they now become part of the immutable blockchain. Finally, block 54d is kept in the Fork DB since new blocks might still be built off of it.
...
<img
    src={require('/static/img/graphviz (9).png').default}
    alt="graph 5"
    width="70%"
    height="automatic"
/>
...
Eventually, both the node and its peer receive new fresh blocks from other peers, which in turn push the blocks to their respective local chains. This causes the head blocks on each chain to advance. Depending on which chain grows first, one of the following actions occurs:
